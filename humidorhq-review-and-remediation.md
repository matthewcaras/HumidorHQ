...
- [x] **M-4 + M-7** ✅ **DONE for new events (CHANGELOG 1.10.4)** — move/remove/receipt events now snapshot location names (`location_name_snapshot()`); the smoking journal falls back to them when the live record is deleted (`smoking_journal_event_location_fallback()`). **Still open:** (a) optional one-time backfill of pre-existing events; (b) the frontend Removal History still resolves live and reads `event.storageLocationId` (should be `fromStorageLocationId`) — needs an `app.js` version bump; (c) "always prefer historical name over current" is an unmade UX choice.
