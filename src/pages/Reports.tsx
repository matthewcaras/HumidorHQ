...
function localYearBounds(year: number) {
  return {
...
function effectiveDateBounds(
  ...
  if (selectedPeriod === 'CURRENT_YEAR') {
    return localYearBounds(new Date().getFullYear())
  }

  if (selectedPeriod === 'PRIOR_YEAR') {
    return localYearBounds(new Date().getFullYear() - 1)
  }
...
function removalEventNoun(type: RemovalReportType, count: number) {
  if (type === 'SMOKED') {
    return `${pluralize(count, 'smoked event')}`
  }

  if (type === 'GIFTED') {
    return `${pluralize(count, 'gifted event')}`
  }

  if (type === 'DISCARDED') {
    return `${pluralize(count, 'discarded event')}`
  }

  return `${pluralize(count, 'removal event')}`
}
...
function missingWarnings(metric: RemovalReportMetric) {
  ...
  if (metric.quantityMissingCost > 0) {
    warnings.push(
      `Cost is unavailable for ${metric.quantityMissingCost} ${pluralize(
  ...
  if (metric.quantityMissingMsrp > 0) {
    warnings.push(
      `MSRP is unavailable for ${metric.quantityMissingMsrp} ${pluralize(
...
function directionLabel(sortBy: RemovalReportSortBy, direction: RemovalReportSortDirection) {
  if (sortBy === 'CIGAR') {
...
function shouldShowRecordedDate(item: RemovalReportItem) {
  return utcDateKey(item.eventDate) !== utcDateKey(item.createdAt)
}
...
function Reports() {
  ...
  function renderSortHeader(headerSortBy: RemovalReportSortBy, label: string) {
    ...
    const activeDirectionLabel = directionLabel(headerSortBy, sortDirection)
  ...
  function renderReportRow(item: RemovalReportItem) {
    return (
      <tr key={item.id}>
        <td>
          <div className="reports-date-cell">
            ...
            {shouldShowRecordedDate(item) ? (
              <span>Recorded {formatDate(item.createdAt)}</span>
            ) : null}
          </div>
        </td>
        <td>
        ...
        <td className="reports-money-cell">{formatMoney(item.totalEventCost)}</td>
        <td className="reports-money-cell">{formatMoney(item.totalEventMsrp)}</td>
        <td className="reports-money-cell">{formatMoney(item.eventSavings)}</td>
      </tr>
    )
  }
    ...
    return (
      <article className="reports-event-card" key={item.id}>
        ...
        <dl className="reports-card-metrics">
          ...
          <div>
            <dt>Total Cost</dt>
            <dd>{formatMoney(item.totalEventCost)}</dd>
          </div>
          <div>
            <dt>Total MSRP</dt>
            <dd>{formatMoney(item.totalEventMsrp)}</dd>
          </div>
          <div>
            <dt>Savings</dt>
            <dd>{formatMoney(item.eventSavings)}</dd>
          </div>
          <div>
            <dt>Source</dt>
        ...
        {shouldShowRecordedDate(item) ? (
          <small className="reports-recorded-date">Recorded {formatDate(item.createdAt)}</small>
        ) : null}
      ...
      <header className="page-header">
        <div className="page-header-copy">
          ...
          <p className="page-subtitle">
            Review smoking, gifting, discarded cigars, and the historical cost of removals.
          </p>
        </div>
      </header>
            ...
            {currentMetric ? (
              <section className="reports-value-section">
                ...
                <div className="reports-value-grid">
                  <MetricCard label="Total Cost" value={formatMoney(currentMetric.totalCost)} />
                  <MetricCard label="Total MSRP" value={formatMoney(currentMetric.totalMsrp)} />
                  <MetricCard label="Total Savings" value={formatMoney(currentMetric.totalSavings)} />
                  <MetricCard
                    label="Average Cost per Cigar"
                    value={formatMoney(currentMetric.averageCostPerCigar)}
                  />
                  <MetricCard
                    label="Average MSRP per Cigar"
                    value={formatMoney(currentMetric.averageMsrpPerCigar)}
                  />
