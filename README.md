...
- `Reports` provides purchase-history summaries by Vendor or cigar manufacturer with an optional Buy Again filter, a Buy Again decision summary and highly rated (8.0+) unevaluated follow-up list, plus filterable smoked, gifted, and discarded removal history by period, custom date range, type, and search. Activity includes signed physical-count adjustments and their append-only reversals. Both purchase-summary views foot to stored purchase `totalPaid`; filtered shares use deterministic cent allocations from line weights without duplicating Purchases-page detail.
